...
post '/outcome/:id' do
  ...
  sql = 'UPDATE outcome_tbl SET name=?, description=? WHERE id = ?'
  @db.execute(sql, [o['name'], o['description'], params[:id]])
end
...
post '/outcome' do
  ...
  sql = 'INSERT INTO outcome_tbl( id, project_id, name, description ) ' \
        "VALUES ( NEXTVAL( 'outcome_seq' ), ?, ?, ? )"
  @db.execute(sql, [o['project_id'], o['name'], o['description']])

  @db.queryForValue("SELECT CURRVAL( 'outcome_seq' )")
end

get '/project/:id/backlog' do
  sql = 'SELECT id, project_id, name, description FROM backlog_tbl WHERE project_id = ?'
  @db.queryForResultset(sql, [params[:id]]).to_json
end
...
post '/backlog/:id' do
  ...
  sql = 'UPDATE backlog_tbl SET name=?, description=? WHERE id = ?'
  @db.execute(sql, [b['name'], b['description'], params[:id]])
end
...
post '/backlog' do
  ...
  sql = 'INSERT INTO backlog_tbl( id, project_id, name, description ) ' \
          "VALUES ( NEXTVAL( 'backlog_seq' ), ?, ?, ? )"
  @db.execute(sql, [b['project_id'], b['name'], b['description']])
...
post '/persona/:id' do
  ...
  sql = 'UPDATE persona_tbl SET name=?, role=? WHERE id = ?'
  @db.execute(sql, [p['name'], p['role'], params[:id]])
end
...
post '/persona' do
  ...
  sql = 'INSERT INTO persona_tbl( id, project_id, name, role ) ' \
          "VALUES ( NEXTVAL('persona_seq'), ?, ?, ? )"
  @db.execute(sql, [p['project_id'], p['name'], p['role']])
...
post '/hypothesis/:id' do
  ...
  sql = 'UPDATE hypothesis_tbl SET outcome_id=?, persona_id=?, description=? WHERE id = ?'
  @db.execute(sql, [p['outcome_id'], p['persona_id'], p['description'], params[:id]])
end
...
post '/hypothesis' do
  ...
  sql = 'INSERT INTO hypothesis_tbl( id, project_id, outcome_id, persona_id, description ) ' \
          "VALUES ( NEXTVAL('hypothesis_seq'), ?, ?, ?, ? )"
  @db.execute(sql, [p['project_id'], p['outcome_id'], p['persona_id'], p['description']])
